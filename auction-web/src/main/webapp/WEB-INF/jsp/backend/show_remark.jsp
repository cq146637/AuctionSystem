...
<body>
                        ...
                        <div class="box">
                            ...
                            <div class="box-body">
                                <table id="example1" class="table table-bordered table-striped">
                                    <thead>
                                    <tr>
                                        <th><input type="checkbox" id="remark_choice" name="choice"></th>
                                        <th>ID</th>
                                        <th>Product ID</th>
                                        <th>Product Name</th>
                                        <th>Pay User ID</th>
                                    ...
                                    <c:forEach var="remark" items="${remarkList}">
                                        <tr>
                                            ...
                                            <td>${remark.id}</td>
                                            <td>${remark.productId}</td>
                                            <td>${productMap[remark.productId]}</td>
                                            <td>${remark.payUserId}</td>
                                            <td>${payUserMap[remark.payUserId]}</td>
                                            <td>${remark.content}</td>
                                            <td>${remark.createTime}</td>
                                            <td><span class="label label-success">Normal</span></td>
                                            <td><i class="fa fa-edit" onclick="editRemark(this);"><div style="width: 35px"></div></i><i class="fa fa-remove" onclick="deleteRemark(this);"></i></td>
                                        </tr>
                                    </c:forEach>
                                    </tbody>
                                    <tfoot>
                                ...
                                <div style="width: 10%; padding-left: 0;">
                                    <button type="button" class="btn btn-block btn-info" onclick="window.location.href='/erp/InsertRemark.html'">添加</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function editRemark(ths) {
            var aid = $(ths).parent().parent().children()[1].innerText;
            var product_name = $(ths).parent().parent().children()[3].innerText;
            var pay_username = $(ths).parent().parent().children()[5].innerText;
            window.location.href = "/erp/QueryRemark.html?id=" + aid + "&productName=" + product_name + "&payUserName=" + pay_username;
        }
        function deleteRemark(ths) {
            var aid = $(ths).parent().parent().children()[1].innerText;
            window.location.href = "/erp/DeleteRemark.html?id=" + aid
        }
        $("#remark_choice").click(function () {
